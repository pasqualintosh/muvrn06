...
class BranchMethods extends React.Component {
  buo = null;
  ...
  shareToTelegram = link => {
    Linking.openURL(`tg://msg?text=${link}`);
  };
  ...
  shareToGmail = link => {
    try {
      ...
      Linking.openURL(
        "https://mail.google.com/mail/?view=cm&su={MUV}&body={" + link + "}"
      );
    } catch (error) {
      console.log(error);
  ...
  openGmail = link => {
    try {
      Linking.openURL("googlegmail://?subject=MUV&body={" + link + "}");
    } catch (error) {
      console.log(error);
      alert(JSON.stringify(error));
    }
  ...
  logCustomEvent = async () => {
    if (!this.buo) await this.createBranchUniversalObject();
    try {
      let branchEvent = new BranchEvent("Test Custom Event Name", this.buo, {
        transactionID: "12344555",
        currency: "USD",
        revenue: 1.5,
        shipping: 10.2,
    ...
    return (
      <View style={styles.container}>
            ...
            {this.state.url != "" ? (
              <View>
                ...
                >
                  ...
                  >
                    <Text style={styles.textSmall}>Share {this.state.url}</Text>
                  </View>
                </TouchableWithoutFeedback>

                <TouchableWithoutFeedback
                  onPress={() => {
                    this.shareToWhatsApp(this.state.url);
                  }}
                >
                  <View
                    style={{
                ...
                <TouchableWithoutFeedback
                  onPress={() => {
                    this.shareToTelegram(this.state.url);
                  }}
                >
                  <View
                    style={{
                ...
                <TouchableWithoutFeedback
                  onPress={() => {
                    this.writeToClipboard(this.state.url);
                  }}
                >
                  <View
                    style={{
                ...
                <TouchableWithoutFeedback
                  onPress={() => {
                    this.shareToGmail(this.state.url);
                  }}
                >
                  <View
                    style={{
        ...
        <ScrollView style={styles.buttonsContainer}>
          ...
          {/* 
          <Button onPress={this.disableTracking}>
            disableTracking (switch on/off)
          </Button> 
          */}
          {/* <Button onPress={this.isTrackingDisabled}>isTrackingDisabled</Button> */}
          {/* 
          <Button onPress={this.createBranchUniversalObject}>
            createBranchUniversalObject
          </Button> 
          */}
          {/* 
          <Button onPress={this.userCompletedAction}>
            userCompletedAction
          </Button> 
          */}
          ...
          {/* 
          <Button onPress={this.redeemRewards.bind(this, "")}>
            redeemRewards
          </Button> 
          */}
          {/* 
          <Button onPress={this.redeemRewards.bind(this, "testBucket")}>
            redeemRewards (with bucket)
          </Button> 
          */}
          {/* <Button onPress={this.loadRewards}>loadRewards</Button> */}
          {/* <Button onPress={this.getCreditHistory}>getCreditHistory</Button> */}
          {/* 
          <Button onPress={this.logStandardEvent}>
            BranchEvent.logEvent (Standard)
          </Button> 
          */}
          {/* 
          <Button onPress={this.logCustomEvent}>
            BranchEvent.logEvent (Custom)
          </Button> 
          */}
        </ScrollView>
      </View>
