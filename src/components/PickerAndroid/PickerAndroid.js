...
export default class PickerAndroid extends Component {
  ...
  _stateFromProps(props) {
    ...
    React.Children.forEach(props.children, (child, index) => {
      if (child.props.value == props.selectedValue) selectedIndex = index;
      items.push({
        value: child.props.value,
        label: child.props.label,
        emoji: child.props.emoji ? child.props.emoji : undefined
      });
    });
  ...
  _renderItems(items) {
    ...
    items.forEach((item, index) => {
      upItems[index] = (
        ...
        >
          {item.label}{" "}
          {item.emoji != undefined ? <Emoji name={item.emoji} /> : ""}
        </Text>
      );
  ...
  _onValueChange() {
    ...
    // controllo se il dato preso esiste prima di salvarlo 
    const check = curItem ? curItem.value ? true : false : false 
    console.log(curItem);
    console.log(this.state.items);
    check && this.state.onValueChange &&
