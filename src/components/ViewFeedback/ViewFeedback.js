...
class ViewFeedback extends React.Component {
  ...
  confirmInputText = () => {
    const feed = this.props.infoAlert;
    // mi serve l'id della domande
    const question = feed[0].questions[0].id;

    this.props.confermModal({ ...this.state, question });
  };
  ...
  continueButtonInput = (feed) => {
    const canSend = feed && feed.length && feed[0].questions.length;
    ...
    return (
      ...
      >
        ...
        >
          <TouchableHighlight
            onPress={() => this.confirmInputText()}
            disabled={!canSend || !this.state.inputValue.length}
            style={{
              width: Dimensions.get("window").width * 0.4,
              height: 46,
              borderRadius: 23,
    ...
    if (this.props.feedbackAnswer) {
      ...
      return (
        ...
        >
          ...
          <Text style={styles.TopQuestionTitle}>
          {strings('id_1_50')}
          </Text>
        </View>
      );
  ...
  renderCointaner = () => {
    if (this.props.type == "Answers") {
      ...
      return this.renderInput(this.props.infoAlert);
    } else {
      return this.questionWithAnswer();
    }
  };
