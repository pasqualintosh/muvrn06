import { UPDATE_STATUS_SCREEN, GET_INFO_SCREEN, UPDATE_TOURNAMENT, UPDATE_PLAYOFF, UPDATE_PLAYOFF_SCREEN } from "./ActionTypes";
    ...
    case UPDATE_PLAYOFF:
      {
        ...
        // se la fase è null non tocco nulla 
        if (!playoffNew.current_phase) {
          return state;
        }
          ...
          for (var key in match) {
            if (match.hasOwnProperty(key)) {
              const newMatch = match[key].map(elem => {
                if (!tournament && (action.city_id == elem.season_match.city_home.id || action.city_id == elem.season_match.city_away.id)) {
                  tournament = { ...elem, key }
                }
                return ({ ...elem })
              })
          ...
          if (index == 4 && matchArray.length) {
            ...
            if (playoffEnd) {
              ...
              if (match.hasOwnProperty("8")) {
                ...
                winHome = firstMatch.total_point_home >= firstMatch.total_point_away
                if (winHome) {
                  winner.firstCity = firstMatch.season_match.city_home
                } else {
                  winner.firstCity = firstMatch.season_match.city_away
                }
              }
              console.log(match)
              if (match.hasOwnProperty(tournament.key)) {
                ...
                // la mia città
                if (action.city_id == ownMatch.season_match.city_home.id) {
                  winner.ownCity = ownMatch.season_match.city_home
                } else {
                  winner.ownCity = ownMatch.season_match.city_away
                }
                // calcolo la mia posizione 
                winHome = ownMatch.total_point_home >= ownMatch.total_point_away
      ...
      break;
    case UPDATE_PLAYOFF_SCREEN:
      {


        const index = action.season_playoff ? action.season_playoff : 0;
        const matchArray = action.payload ? action.payload : []
        let playoff = state.playoff ? state.playoff : [[], [], [], [], []]
        ...
        if (index == 4 && matchArray.length) {
          ...
          if (playoffEnd) {
            ...
            // prendo la prima squadra che ha vinto il primo match e il mio match
              ...
              // cerco la mia squadra
            
            for (var indexMatch = 0; indexMatch < matchArray.length; indexMatch++)  {
              ownMatch = matchArray[indexMatch]
              // la mia città
              if (action.city_id == ownMatch.season_match.city_home.id) {
                winner.ownCity = ownMatch.season_match.city_home
                // calcolo la mia posizione 
              ...
              if (winHome) {
                winner.ownPosition = (indexMatch * 2) + 1
              } else {
                winner.ownPosition = (indexMatch * 2) + 2
              }
              break;
              } else if (action.city_id == ownMatch.season_match.city_away.id) {
                winner.ownCity = ownMatch.season_match.city_away
                // calcolo la mia posizione 
              winHome = ownMatch.total_point_home >= ownMatch.total_point_away
        ...
        // modifico l'array in array di array
        // if (week == 2) {
        //   playoffIndex = 4
        // } else if (week == 2) {
        //   playoffIndex = 2
