...
import { getTutorialMetroState, getTutorialCarPoolingState, getTutorialLiveState } from "../../domains/login/Selectors";
...
class TrackAuto extends React.Component {
  constructor(props) {
    super(props);
  ...
  completeTutorialCarPooling = () => {
    if (this.state.typeAlert == 'Metro') {
      this.props.dispatch(completeTutorial("tutorialMetro"))
    } else if (this.state.typeAlert == 'Rules') {
      this.props.dispatch(completeTutorial("tutorialCarPooling"))  
    } else if (this.state.typeAlert == 'AlertHonor') {
      Platform.OS !== "ios"
      ? Platform.Version < 23
  ...
  componentWillMount() {
    ...
    console.log("montato");
    if (
      store.getState().tracking.PreviousRoute
        ? store.getState().tracking.PreviousRoute.length
        : 0
    ) {
  ...
  alertModal = async () => {
    ...
    if (
      ...
      this.openAlert("AlertHonor");
    } else if (
      (manufacturer === "OnePlus" || model === "OnePlus") &&
      parseInt(systemVersion) > 8
    ) {
      this.openAlert("AlertOnePlus");
    }
  };

  addNotificationIcon = async () => {
    try {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (
        !this.props.tutorialCarPooling &&
        this.state.time < 10000 && 
        this.props.activityChoice.type === "Carpooling"
      ) {
 ...
 */
  ...
  renderPoolingPartners() {
    return (
      <TouchableOpacity
        onPress={() => {
          this.props.navigation.navigate("PoolingUsersScreen");
        }}
        style={{
          position: "absolute",
          shadowRadius: 5,
          ...
          shadowOffset: { width: 0, height: 5 },
          shadowOpacity: 0.5,
          top: (Dimensions.get("window").width / 2 + 30) / 2,
          left: 17,
          elevation: 2,
          borderRadius: 18,
          height: 36,
          ...
          alignItems: "center",
          backgroundColor: "#fff",
          borderColor: `rgba(${this.state.r}, ${this.state.g}, ${this.state.b}, 1)`,
          borderWidth: 0.5
        }}
      >
        <View
        ...
        >
          <Text
            style={[
              styles.poolingPartnerText,
              {
                color: `rgba(${this.state.r}, ${this.state.g}, ${this.state.b}, 1)`
              }
            ]}
          >
            {this.props.numUserPooling}
          </Text>
        </View>
      </TouchableOpacity>
    );
  ...
  renderAddPooling() {
    return (
      <TouchableOpacity
        onPress={() => {
          this.props.navigation.navigate("PoolingRadarScreen");
        }}
        style={{
          position: "absolute",
          shadowRadius: 5,
  ...
  renderNoPoolingUsers() {
    return (
      <View style={{ marginTop: 50 }}>
        ...
        >
          <Image
            style={styles.poolingImage}
            source={require("./../../assets/images/carpooling_add.png")}
            resizeMode={"contain"}
          ></Image>
        </TouchableOpacity>
      </View>
  ...
  renderModalSplitImageWithPooling(index, points, bg) {
    return (
      <View style={{ marginTop: 50 }}>
        {this.renderPoolingPartners()}
        {this.renderAddPooling()}
        <ImageBackground
          style={styles.circle6}
          source={modalSplitImage[index]}
          resizeMode={"contain"}
  ...
  liveNotPooling = (time, distanceWithComma, typeWeather, kmTrovati) => {
    ...
    const pointsBackend =
      this.props.subTrip.points + this.state.totPointsBackend;
    return (
      <Aux>
       
          {this.renderModalSplitImage(
              this.state.modalSplitIndex,
              pointsFake > pointsBackend ? pointsFake : pointsBackend,
        ...
        >
          ...
          >
            ...
            {/* <View style={{ alignContent: "center", flex: 1 }}>
              ...
              >
                {this.props.numUserPooling ? "+ " + this.props.numUserPooling * 5 + "%" : ""}
              </Text>
            </View>  */}
  ...
  liveCarPooling = (time, distanceWithComma, typeWeather, kmTrovati) => {
    let pointsFake = calcolatePointsFake(
      this.props.distanceLive,
      0,
      this.props.activityChoice.type,
    ...
    return (
      <Aux>
        {this.props.numUserPooling
          ? this.renderModalSplitImageWithPooling(
              this.state.modalSplitIndex,
              pointsFake > pointsBackend ? pointsFake : pointsBackend,
              `rgba(${this.state.r}, ${this.state.g}, ${this.state.b}, 1)`
            )
          : this.renderNoPoolingUsers()}
  ...
  getNumSubTripsFromTrips = props => {
    ...
    for (i = lengthPreviousRoute - 1; i >= 0; i--) {
      const numSubTripNext = store.getState().tracking.PreviousRoute[i]
        .numSubTrip;
      ...
      return (
        ...
        >
          ...
          <AlertCarPooling
            ...
            confermModal={this.completeTutorialCarPooling}
          ...
          this.props.activityChoice.coef === 1200
            ...
            : this.props.activityChoice.type == "Carpooling" ? this.liveCarPooling(
                time,
                distanceWithComma,
                typeWeather,
                kmTrovati
              ) : this.liveNotPooling( time,
                distanceWithComma,
                typeWeather,
                kmTrovati)}
        </View>
...
export const modalSplitImage = {
  ...
  5: require("./../../assets/images/live_carpooling.png")
};
...
const withActivity = connect((state, props) => {
  ...
  return {
    ...
    tutorialCarPooling: getTutorialCarPoolingState(state),
    tutorialLive: getTutorialLiveState(state),
    numUserPooling: getGroupPoolingLenghtState(state)
  };
});

export default withActivity(TrackAuto);
