...
import { newBestScore } from "./../../domains/login/ActionCreators";
...
const FLOOR_HEIGHT = 48 + 35;
const FLOOR_Y = 11 + 35;
const HOOP_Y = Dimensions.get("window").height - 300;
const NET_HEIGHT = 6;
const NET_WIDTH = 83;
const NET_Y = Dimensions.get("window").height - 216 - 73;
...
class BasketBallScreen extends Component {
  constructor(props) {
    ...
    this.state = {
      // x: Dimensions.get("window").width - radius - 48,
  ...
  componentDidMount() {
    if (Platform.OS == "ios") {
      ...
      this.interval = setInterval(this.update.bind(this), 1000 / 60);
        ...
        // su android a 30 fps 
      this.interval = setInterval(this.update.bind(this), 1000 / 140);
  ...
  performAnimation = () => {
    //...
    this.interval = setTimeout(this.update.bind(this), 1000 / 60)
  }
    ...
    if (this.state.scored === null) {
      if (
        ...
        if (
          ...
          if (nextState.score > this.props.bestScore) {
            this.props.dispatch(newBestScore(nextState.score))
            // dispatch
            // nextState.bestScore = nextState.score
          }
    ...
    return (
      <SafeAreaView style={styles.container}>
       <LinearGradient
        ...
        />
        <Hoop y={HOOP_Y} bestScore={this.props.bestScore}/>
        ...
        {this.renderNet(this.state.lifecycle !== LC_STARTING)}
        {this.renderFloor(this.state.vy > 0)}
        <Emoji y={NET_Y + 30} scored={this.state.scored} />
        </LinearGradient>
      </SafeAreaView>
    );
  }
...
const withBestScore = connect(state => {
  // prendo le info del match corrente
  return {
    bestScore: state.login.bestScore ? state.login.bestScore : 0
  };
});
...
export default withBestScore(BasketBallScreen);
