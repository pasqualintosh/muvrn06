...
class GenericWebViewScreen extends React.Component {
  constructor() {
    super();
    this.state = {
      url: "", 
  ...
  componentWillMount() {
    Tracker.trackScreenView("GenericWebViewScreen.js");
    trackScreenView("GenericWebViewScreen.js");

    const url = this.props.navigation.getParam("url", "");
    const dataUrl = this.props.navigation.getParam("dataUrl", {});

    const getParams = Object.keys(dataUrl).reduce((total, item, index) => {
      // if (
      //   index
      // ) {
      if (dataUrl.hasOwnProperty(item)) {
        if (total.length) {
          return total + "&" + item + "=" + dataUrl[item].toLowerCase();
        } else {
          return total + "?" + item + "=" + dataUrl[item].toLowerCase();
        }
  ...
  static navigationOptions = ({ navigation }) => {
    return {
      headerTitle: (
        ...
        >
          {navigation.getParam("titleUrl", "Web")}
        </Text>
      )
    };
  };
        ...
        >
          ...
          <View style={{ flex: 1, backgroundColor: "#fff" }}>
            ...
            />
              ...
              source={{
                uri: this.state.url + this.state.dataUrl
              }}
